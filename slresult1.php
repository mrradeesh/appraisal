...
<?php
	...
	$search=$k['Empid'];//when the name is given to find out the empid
	$name=$k['Name'];
...
$n=0;
	...
	{
		$total=$total+base64_decode($i['Total']);
		$y=$y+base64_decode($i['Yes']);//$total*100;
		$n=$n+base64_decode($i['No']);//$total*100;
	}
	$total=($total*$tq1);
	$y=$y/$total*100;
	$n=$n/$total*100;
